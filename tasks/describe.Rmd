...
write.csv('/home/jrl/rcrs/data/yates_oats.csv')
```
...
Data frames can be saved as plain text using `write.csv()`. As a minimum it is necessary to provide the (1) name of the data frame to be saved and (2) path to file where it will be saved. Save data frame oats to a file called *oats.csv* somewhere on your computer where you can find it.

```{r, eval = F}
write.csv(oats, 'oats.csv')
```
